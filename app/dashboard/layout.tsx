    ...
    <NotificationContext.Provider value={notificationContextValue}>
      <div className="flex min-h-screen bg-gradient-to-br from-black via-gray-950 to-orange-950/20 overflow-hidden">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          className="fixed top-4 left-4 z-50 md:hidden bg-black/80 text-orange-500 p-2.5 rounded-full shadow-lg shadow-orange-500/20 border border-orange-500/40 hover:bg-black/90 hover:border-orange-500/60 transition-all duration-200"
        ...
        {/* Sidebar */}
        <motion.aside
          className="fixed left-0 top-0 bottom-0 w-[85vw] max-w-[280px] bg-black/70 backdrop-blur-xl border-r border-orange-500/20 overflow-y-auto z-40 shadow-xl shadow-orange-900/10"
              ...
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className={`flex items-center justify-center w-10 h-10 rounded-lg mr-3 ${isActive ? 'bg-orange-500/20 text-orange-400' : 'bg-gray-800/60 text-gray-400 group-hover:text-orange-400 group-hover:bg-orange-500/10'} transition-colors`}>
          ...
          <div className="px-3 py-2 mt-8">
            ...
            <nav className="space-y-1.5">
              <Link href="#" onClick={(e) => { e.preventDefault(); setSelectedFeature('Help & Guides'); setShowComingSoon(true); }}>
                <motion.div
                  whileHover={{ x: 3 }}
                  className="flex items-center px-4 py-3.5 text-sm font-medium rounded-xl text-gray-300 hover:bg-orange-500/5 hover:text-orange-300 group transition-all duration-200"
                >
                  <div className="flex items-center justify-center w-10 h-10 rounded-lg mr-3 bg-gray-800/60 text-gray-400 group-hover:text-orange-400 group-hover:bg-orange-500/10 transition-colors">
        ...
        <div className={`flex-1 ${sidebarOpen ? 'md:ml-72' : 'ml-0'} transition-all duration-300 overflow-x-hidden`}>
          {/* Header */}
          <header className="sticky top-0 z-10 bg-black/60 backdrop-blur-xl border-b border-orange-500/20 px-3 sm:px-5 py-3 flex justify-between items-center shadow-md shadow-black/20">
            <div className="w-8 md:w-auto">  {/* Placeholder for mobile spacing */}
              <motion.button
                onClick={toggleSidebar}
            ...
            <div className="flex items-center ml-auto space-x-3 sm:space-x-5">
              <motion.div
                className="relative"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                  ...
                  {showNotifications && (
                    <>
                      ...
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="fixed sm:absolute right-2 sm:right-0 top-14 sm:mt-2 w-[calc(100vw-16px)] sm:w-80 max-h-[80vh] max-w-[320px] bg-gray-900 border border-orange-500/20 rounded-lg shadow-lg shadow-black/30 z-50 overflow-hidden"
                      >
                        ...
                        <div className="max-h-[calc(80vh-60px)] overflow-y-auto scrollbar-thin scrollbar-thumb-orange-500/20 scrollbar-track-transparent">
                          {notificationList.length === 0 ? (
                            ...
                            notificationList.map(notification => (
                              <div 
                                key={notification.id} 
                                className={`p-2.5 border-b border-gray-800 hover:bg-gray-800/50 transition-colors cursor-pointer ${notification.read ? 'opacity-70' : ''}`}
                              ...
                              >
                                <div className="flex items-start">
                                  ...
                                  <div className="flex-1 overflow-hidden">
                                    <div className="font-medium text-white text-sm truncate">{notification.title}</div>
                                  ...
                                  {!notification.read && (
                                    <div className="w-2 h-2 bg-orange-500 rounded-full flex-shrink-0 ml-1" />
                                  )}
                                </div>
                              </div>
                            ))
          ...
          <main className="px-3 py-4 sm:px-5 sm:py-5">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="pb-16 sm:pb-8" /* Add bottom padding for mobile */
              >
                {children}
              </motion.div>
            </AnimatePresence>
