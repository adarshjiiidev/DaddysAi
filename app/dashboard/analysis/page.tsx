...
import { Search, TrendingUp, TrendingDown, Clock, Calendar, Volume2, CandlestickChart, ChevronDown, BarChart4, BarChart2, Settings, X, Star, ArrowRight } from 'lucide-react';
...
export default function AnalysisPage() {
  ...
  useLayoutEffect(() => {
    ...
    if (!chartRef.current) {
      ...
      const calculateResponsiveHeight = () => {
        ...
        // Set chart height based on screen size - use larger percentages
        let chartHeight;
        if (window.innerWidth < 640) { // sm breakpoint
          chartHeight = vh * 0.7; // 70% of viewport height on mobile
        } else if (window.innerWidth < 768) { // md breakpoint
          chartHeight = vh * 0.75; // 75% of viewport height on small tablets
        } else if (window.innerWidth < 1024) { // lg breakpoint
          chartHeight = vh * 0.8; // 80% of viewport height on tablets
        } else {
          chartHeight = vh * 0.85; // 85% of viewport height on desktops
        }
        ...
        // Add minimal padding to the chart container
        chartContainerRef.current.style.padding = '8px';
        
        return chartHeight;
      };
      
      // Calculate initial height
      const initialHeight = calculateResponsiveHeight();
      ...
      const chartOptions: DeepPartial<ChartOptions> = {
        ...
        width: chartContainerRef.current.clientWidth - 16, // Account for minimal padding
        height: initialHeight - 16, // Account for minimal padding
        grid: {
          vertLines: { 
            color: 'rgba(42, 46, 57, 0.6)', // More visible grid
        ...
        crosshair: {
          ...
          vertLine: {
            width: 1,
            color: 'rgba(255, 153, 0, 0.6)', // Orange crosshair
            style: LineStyle.Dashed,
            labelBackgroundColor: 'rgba(255, 153, 0, 0.9)',
            labelVisible: true,
          },
          horzLine: {
            width: 1,
        ...
        rightPriceScale: {
          borderColor: 'rgba(197, 203, 206, 0.3)', // Softer border
          borderVisible: true,
          scaleMargins: {
            top: 0.15, // More space at top
          ...
          entireTextOnly: true,
          borderWidth: 1,
          textColor: 'rgba(255, 255, 255, 0.8)', // Slightly softer text
        },
        timeScale: {
          borderColor: 'rgba(197, 203, 206, 0.3)', // Softer border
          timeVisible: true,
          ...
          borderVisible: true,
          rightOffset: 0, // Remove space on the right to avoid gaps
          barSpacing: 4, // Even narrower bars to fit more data and reduce gaps
          minBarSpacing: 1, // Minimum spacing between bars
          fixLeftEdge: true,
          fixRightEdge: true,
          lockVisibleTimeRangeOnResize: true,
          uniformDistribution: true, // Ensure uniform spacing between bars
      ...
      try {
        ...
        if (chartData.length > 0) {
          ...
          // Calculate the visible range based on actual data points
          if (formattedData.length > 0) {
            // Find the earliest and latest time in the data
            const times = formattedData.map(item => Number(item.time));
            const earliestTime = Math.min(...times);
            const latestTime = Math.max(...times);
            
            // Add minimal padding (1% on each side) to reduce gaps
            const timeRange = latestTime - earliestTime;
            const padding = timeRange * 0.01;
            
            // Set the visible range to show all data points with minimal padding
            chartRef.current.timeScale().setVisibleRange({
              from: Math.floor(earliestTime - padding),
              to: Math.ceil(latestTime + padding)
            });
            
            // Force chart to fit content immediately
            chartRef.current.timeScale().fitContent();
            
            // Also fit content after a small delay to ensure proper rendering
            setTimeout(() => {
              chartRef.current.timeScale().fitContent();
            }, 100);
          } else {
            // Fallback to market hours if no data
    ...
    // Handle resize immediately to fill available space
    if (chartRef.current) {
      ...
      // Force chart to fit content after resize
      setTimeout(() => {
        chartRef.current.timeScale().fitContent();
      }, 100);
    }
  ...
  // Add window resize handler to ensure chart always fills available space
  useEffect(() => {
    const handleResize = () => {
  ...
  useEffect(() => {
    ...
    const uniqueData = ensureUniqueTimeOrdering(formattedData);
    candlestickSeriesRef.current.setData(uniqueData);

    // Calculate the visible range based on actual data points
    if (uniqueData.length > 0) {
      // Find the earliest and latest time in the data
      const times = uniqueData.map(item => Number(item.time));
      const earliestTime = Math.min(...times);
      const latestTime = Math.max(...times);
      ...
      // Set the visible range to show all data points with padding
      chartRef.current.timeScale().setVisibleRange({
        from: Math.floor(earliestTime - padding),
        to: Math.ceil(latestTime + padding)
      });
  ...
  useEffect(() => {
    ...
    const calculateResponsiveHeight = () => {
      ...
      // Get the parent container's height (the flex-1 div)
      ...
      // Calculate available height (accounting for navbar and other UI elements)
      const navbarHeight = 48; // 3rem for the navbar
      const topControlsHeight = 80; // Approximate height of top controls
      const containerPadding = 32; // Padding of the container (16px * 2)
      
      // Set chart height based on screen size with higher percentages
      let chartHeight;
      if (window.innerWidth < 640) { // sm breakpoint
        chartHeight = vh - navbarHeight - topControlsHeight - containerPadding;
      } else if (window.innerWidth < 768) { // md breakpoint
        chartHeight = vh - navbarHeight - topControlsHeight - containerPadding;
      } else if (window.innerWidth < 1024) { // lg breakpoint
        chartHeight = vh - navbarHeight - topControlsHeight - containerPadding;
      ...
      // Apply the calculated height to the chart container
      parentContainer.style.height = `${chartHeight}px`;
    ...
    const handleResize = debounce(() => {
      if (chartContainerRef.current && chartRef.current) {
        const dimensions = calculateResponsiveHeight();
        if (dimensions) {
          chartRef.current.applyOptions(dimensions);
          
          // Force time scale to fit all data after resize
          chartRef.current.timeScale().fitContent();
        }
      }
    }, 100); // 100ms debounce
    ...
    // Force time scale to fit all data after initial render
    if (chartRef.current) {
      chartRef.current.timeScale().fitContent();
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3rem)]">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-2 sm:p-4 border-b border-gray-800 bg-transparent gap-3 sm:gap-0">
        {/* Symbol Dropdown */}
        <div className="relative w-full sm:w-auto" ref={dropdownRef}>
          <button
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="flex w-full sm:w-auto items-center justify-between sm:justify-start space-x-3 bg-gray-800/60 backdrop-blur-sm text-white px-4 py-3 rounded-xl border border-gray-700/50 shadow-md hover:bg-gray-800/80 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 transition-all duration-300"
          >
            <div className="flex items-center">
              <CandlestickChart className="h-5 w-5 text-orange-400 mr-2 hidden sm:block" />
              <span className="font-medium truncate max-w-[150px] sm:max-w-none">{popularSymbols.find(s => s.symbol === symbol)?.name || symbol}</span>
              <span className="text-sm text-gray-300 ml-2 bg-gray-800/50 px-2 py-0.5 rounded-full">{symbol}</span>
            </div>
            <ChevronDown className={`h-5 w-5 text-orange-400 transition-transform ${isDropdownOpen ? 'transform rotate-180' : ''}`} />
          </button>

          {isDropdownOpen && (
            <div className="absolute z-50 mt-3 w-full sm:w-96 rounded-xl bg-gray-900/95 backdrop-blur-sm border border-gray-700/50 shadow-2xl overflow-hidden animate-slideIn origin-top-left">
              <div className="p-4 bg-gradient-to-r from-gray-900 to-gray-800/80">
                <div className="relative mb-4">
                  <div className="relative group">
                    <input
                      ...
                      className="w-full bg-gray-800/70 text-white px-4 py-3 rounded-xl pr-10 border border-gray-700/50 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 shadow-inner transition-all duration-300 placeholder-gray-400"
                      autoFocus
                    />
                    <div className="absolute right-3 top-3 text-orange-400 h-5 w-5 group-hover:animate-pulse">
                      <Search className="h-full w-full" />
                    </div>
                    {searchQuery && (
                      <button 
                        onClick={() => setSearchQuery('')}
                        className="absolute right-10 top-3 text-gray-400 hover:text-white transition-colors duration-200"
                      >
                        <X className="h-5 w-5" />
                      </button>
                    )}
                  ...
                  <span className="font-medium text-gray-300 flex items-center gap-2">
                    {searchQuery === '' ? (
                      <>
                        ...
                        <span className="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent">Popular Symbols</span>
                      </>
                    ) : (
                      <>
                        <Search className="h-4 w-4 text-orange-400" />
                        <span className="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent">Search Results</span>
                      </>
                    )}
                  </span>
              ...
              <div className="max-h-60 sm:max-h-80 overflow-y-auto py-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
                {filteredSymbols.length > 0 ? (
                  filteredSymbols.map((item) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-gray-800/70 transition-all duration-200 border-l-0 hover:border-l-2 hover:border-orange-500/50 ${
                        symbol === item.symbol ? 'bg-orange-500/10 border-l-2 border-orange-500 text-white' : 'text-gray-300'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className={`font-medium truncate ${symbol === item.symbol ? 'text-orange-400' : ''}`}>{item.name}</div>
                          <div className="text-xs mt-0.5 flex items-center gap-1">
                            <span className={`px-1.5 py-0.5 rounded-full ${symbol === item.symbol ? 'bg-orange-500/20 text-orange-300' : 'bg-gray-800 text-gray-400'}`}>{item.symbol}</span>
                          </div>
                        </div>
                        {symbol === item.symbol ? (
                          <div className="flex items-center justify-center h-8 w-8 rounded-full bg-orange-500/20">
                            <CandlestickChart className="h-4 w-4 text-orange-400 animate-pulse" />
                          </div>
                        ) : (
                          <div className="flex items-center justify-center h-8 w-8 rounded-full bg-gray-800/50 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  ...
                  <div className="text-center py-10 px-4">
                    <div className="bg-gray-800/50 rounded-lg p-4 flex flex-col items-center">
                      ...
                      <p className="text-gray-400 font-medium">No symbols found</p>
        ...
        <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto space-x-2 sm:space-x-4">
          ...
          {ltpData?.direction && (
            <div className={`flex items-center gap-1 sm:gap-2 text-xs sm:text-sm font-medium rounded-md px-2 sm:px-3 py-1 sm:py-1.5 ${
              ...
              <span className="hidden xs:inline">{ltpData.direction}</span>
            </div>
          )}
          ...
          <div className="relative" ref={indicatorsMenuRef}>
            <button
              ...
              className={`flex items-center space-x-2 sm:space-x-3 bg-gray-800/60 backdrop-blur-sm text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg border shadow-md hover:bg-gray-800/80 ${
            ...
            >
              ...
              <span className="hidden sm:inline font-medium">
                {indicators.some(ind => ind.enabled) ? (
                  <span className="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent">Indicators</span>
                ) : (
                  "Indicators"
                )}
              </span>
            ...
            {isIndicatorsMenuOpen && (
              <div className="absolute right-0 z-50 mt-3 w-60 sm:w-72 rounded-xl bg-gray-900/95 backdrop-blur-sm border border-gray-700/50 shadow-2xl animate-slideIn origin-top-right overflow-hidden">
                <div className="p-3 sm:p-4 border-b border-gray-800/70 bg-gradient-to-r from-gray-900 to-gray-800/80">
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-white flex items-center gap-1.5 sm:gap-2">
                      <BarChart2 className="h-4 w-4 sm:h-5 sm:w-5 text-orange-400" />
                      <span className="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent text-sm sm:text-base">Chart Indicators</span>
                    </h3>
                    <button 
                      onClick={() => setIsIndicatorsMenuOpen(false)}
                      className="text-gray-400 hover:text-white transition-colors duration-200 bg-gray-800/50 rounded-full p-1 hover:bg-gray-700/50"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                ...
                <div className="py-3 max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">{indicators.map((indicator, index) => (
                  <div 
                    key={indicator.id}
                    id={`indicator-${indicator.id}`}
                    className={`flex items-center justify-between px-4 py-3 hover:bg-gray-800/50 transition-all duration-300 ${
                      indicator.enabled ? 'bg-gray-800/40 border-l-3 border-orange-500/80' : ''
                    }`}
                  >
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      />
                      <div className={`relative w-12 h-6 rounded-full transition-all duration-300 ${indicator.enabled ? 'bg-gradient-to-r from-orange-500 to-orange-400' : 'bg-gray-700'}`}>
                        <span className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full shadow-md transform transition-transform duration-300 ease-in-out ${indicator.enabled ? 'translate-x-6 bg-white' : 'translate-x-0 bg-gray-300'}`}></span>
                        {indicator.enabled && <span className="absolute inset-0 rounded-full animate-pulse-subtle opacity-70 bg-orange-400/30"></span>}
                      </div>
                    </label>
                  </div>
                ))}</div>
      ...
      {/* Chart Container */}
      <div className="flex-1 relative w-full h-full bg-transparent p-2 sm:p-4 flex flex-col flex-grow">
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl overflow-hidden shadow-lg bg-gradient-to-b from-gray-900/80 to-gray-950/90 border border-gray-800/50">
            <div className="animate-spin rounded-full h-10 w-10 sm:h-14 sm:w-14 border-4 border-orange-500/20 border-t-orange-500 mb-3 sm:mb-4"></div>
            <div className="text-sm sm:text-base text-gray-300 font-medium bg-gray-900/70 px-4 py-2 rounded-full">
              <span className="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent">Loading chart data</span>
              <span className="animate-pulse">...</span>
            </div>
          </div>
        ) : (
          <>
            <div className="flex-1 w-full h-full relative rounded-xl overflow-hidden shadow-lg bg-gradient-to-b from-gray-900/80 to-gray-950/90 border border-gray-800/50">
              <div ref={chartContainerRef} className="w-full h-full bg-transparent flex-1"></div>
            </div>
            
            {/* OHLC Info Card - enhanced */}
            <div 
              className={`absolute top-6 sm:top-8 right-6 sm:right-8 z-50 bg-gray-900/95 backdrop-blur-sm border border-gray-700/50 rounded-lg p-3 sm:p-4 shadow-xl transition-all duration-300 text-xs sm:text-sm ${
                showOhlcCard ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'
              }`}
            >
              <div className="flex items-center justify-between mb-2 sm:mb-3 border-b border-gray-800/70 pb-2">
                <div className="font-medium text-white flex items-center gap-1.5 sm:gap-2">
                  <BarChart4 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-orange-400" />
                  <span className="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent font-semibold">OHLC Data</span>
                </div>
                <div className="text-xs font-medium text-gray-300 bg-gray-800/50 px-2 py-0.5 rounded-full">{ohlcData?.time || ''}</div>
              </div>
              <div className="grid grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-2 sm:gap-y-2.5">
                <div className="flex items-center group transition-all duration-200 hover:bg-gray-800/30 p-1 rounded-md">
                  <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-blue-500 mr-2 group-hover:animate-pulse"></div>
                  <span className="text-gray-300 group-hover:text-gray-200">Open:</span>
                  <span className="text-white font-medium ml-1.5 sm:ml-2 group-hover:text-blue-300">
                    {ohlcData ? formatPrice(ohlcData.open) : '-'}
                  </span>
                </div>
                <div className="flex items-center group transition-all duration-200 hover:bg-gray-800/30 p-1 rounded-md">
                  <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-green-500 mr-2 group-hover:animate-pulse"></div>
                  <span className="text-gray-300 group-hover:text-gray-200">High:</span>
                  <span className="text-white font-medium ml-1.5 sm:ml-2 group-hover:text-green-300">
                    {ohlcData ? formatPrice(ohlcData.high) : '-'}
                  </span>
                </div>
                <div className="flex items-center group transition-all duration-200 hover:bg-gray-800/30 p-1 rounded-md">
                  <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-red-500 mr-2 group-hover:animate-pulse"></div>
                  <span className="text-gray-300 group-hover:text-gray-200">Low:</span>
                  <span className="text-white font-medium ml-1.5 sm:ml-2 group-hover:text-red-300">
                    {ohlcData ? formatPrice(ohlcData.low) : '-'}
                  </span>
                </div>
                <div className="flex items-center group transition-all duration-200 hover:bg-gray-800/30 p-1 rounded-md">
                  <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-orange-500 mr-2 group-hover:animate-pulse"></div>
                  <span className="text-gray-300 group-hover:text-gray-200">Close:</span>
                  <span className="text-white font-medium ml-1.5 sm:ml-2 group-hover:text-orange-300">
