...
const DEFAULT_LTP_USERNAME = "classesdaddys@gmail.com";
const DEFAULT_LTP_PASSWORD = "Schools@12345";
...
// Helper function to get user's LTP credentials
async function getUserLTPCredentials(request: NextRequest) {
  try {
    ...
    if (!user || !user.ltpCredentials) {
      return null;
    }

    return {
      username: user.ltpCredentials.username,
      password: user.ltpCredentials.password
    };
  } catch (error) {
    console.error('Error getting user LTP credentials:', error);
    return null;
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    // Try to get user's LTP credentials
    const userCredentials = await getUserLTPCredentials(request);
    
    // Use user credentials if available, otherwise use defaults
    const LTP_USERNAME = userCredentials?.username || DEFAULT_LTP_USERNAME;
    const LTP_PASSWORD = userCredentials?.password || DEFAULT_LTP_PASSWORD;
    ...
    // If no expiry is specified, fetch available expiry dates and choose the nearest one
      ...
      // For NIFTY, use the fixed weekly Thursday expiry format
      if (symbol === 'NIFTY') {
        ...
        console.log(`Using weekly Thursday expiry for NIFTY: ${expiry}`);
      } else {
        // For other symbols, fetch available expiry dates
        const expiryDates = await fetchExpiryDates(mappedSymbol);
        
        if (expiryDates.length > 0) {
          // For other symbols, use the monthly expiry
          expiry = findNearestExpiry(expiryDates, false);
        } else {
          // Fallback to the last Thursday of the month
