...
export default function OptionChain({ spotPrice, onStrikeSelect }: OptionChainProps) {
  ...
  return (
    <div ref={containerRef} className="bg-gray-800/50 rounded-xl p-2 sm:p-3 md:p-4 overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 sm:mb-4 gap-2 sm:gap-0">
        <div>
          ...
          <div className="text-xs sm:text-sm flex flex-wrap items-center gap-1 sm:gap-2">
            ...
            {showImaginaryLine && (
              <span className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 bg-blue-500/20 border border-blue-500/50 rounded-full flex items-center">
                <span className="text-blue-300 mr-0.5 sm:mr-1">Between</span>
                <span className="text-white font-medium">{lowerStrike.toFixed(1)}</span>
                <ArrowUpDown className="h-2.5 w-2.5 sm:h-3 sm:w-3 mx-0.5 sm:mx-1 text-blue-300" />
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          <select
            ...
            className="bg-gray-700 text-white rounded-lg px-2 sm:px-3 py-1 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      {error && (
        <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-2 sm:p-3 mb-3 sm:mb-4 flex items-center text-red-300 text-xs sm:text-sm">
      ...
      {optionData.length === 0 && !error ? (
        <div className="flex items-center justify-center h-40">
          <div className="animate-pulse text-gray-400 text-sm sm:text-base">Loading option chain data...</div>
        </div>
      ) : (
        <div className="overflow-auto -mx-2 sm:mx-0" style={{ maxHeight: `${containerHeight}px` }}>
          <table className="w-full text-xs sm:text-sm">
            <thead className="sticky top-0 bg-gray-900 z-10">
              <tr className="text-gray-400 border-b border-gray-700">
                <th className="px-1.5 sm:px-3 md:px-4 py-1.5 sm:py-2 text-center text-[10px] sm:text-xs" colSpan={6}>CALLS</th>
                <th className="px-1.5 sm:px-3 md:px-4 py-1.5 sm:py-2 text-center text-[10px] sm:text-xs">Strike</th>
                <th className="px-1.5 sm:px-3 md:px-4 py-1.5 sm:py-2 text-center text-[10px] sm:text-xs" colSpan={6}>PUTS</th>
              </tr>
              <tr className="text-gray-400 text-[9px] sm:text-[10px] md:text-xs bg-gray-900">
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">IV</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">Delta</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">Vol</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">OI</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">B/A</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">LTP</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 font-bold">Strike</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">LTP</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">B/A</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">OI</th>
                <th className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2">Vol</th>
            ...
            <tbody>
              ...
              {showImaginaryLine && (
                <tr key="imaginary-line-header" className="border-none">
                  <td colSpan={13} className="px-0 py-1 sm:py-2 text-center">
              ...
              {filteredOptionData.map((row, index, arr) => {
                ...
                return (
                  <>
                    ...
                    >
                      {/* Call Side */}
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{row.call_options.option_greeks.iv.toFixed(1)}%</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{row.call_options.option_greeks.delta.toFixed(2)}</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{formatNumber(row.call_options.market_data.volume)}</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{formatNumber(row.call_options.market_data.oi)}</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">
                        {row.call_options.market_data.bid_price.toFixed(1)}/{row.call_options.market_data.ask_price.toFixed(1)}
                      </td>
                      <td 
                        className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-white text-center cursor-pointer hover:text-blue-400 group relative text-[9px] sm:text-xs"
                        onClick={() => onStrikeSelect?.(row.strike_price, 'CE', row.call_options.market_data.ltp)}
                      >
                        <div className="flex items-center justify-center">
                          {row.call_options.market_data.ltp.toFixed(1)}
                          <span className="absolute -top-6 sm:-top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-[8px] sm:text-xs text-gray-300 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10">
                            Click to trade
                          </span>
                        </div>
                      </td>
                      ...
                      {/* Strike Price */}
                      <td className={`px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-center font-semibold text-[9px] sm:text-xs ${
                        ...
                        {isLowerBracket(row.strike_price) && (
                          <div className="text-[8px] sm:text-[10px] text-blue-400 font-normal flex items-center justify-center mt-0.5 sm:mt-1">
                            <span className="inline-block w-1.5 sm:w-2 h-1.5 sm:h-2 bg-blue-500 rounded-full mr-0.5 sm:mr-1"></span>
                            Lower
                          </div>
                        )}
                      ...
                      >
                        <div className="flex items-center justify-center">
                          {row.put_options.market_data.ltp.toFixed(1)}
                          <span className="absolute -top-6 sm:-top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-[8px] sm:text-xs text-gray-300 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10">
                            Click to trade
                          </span>
                        </div>
                      ...
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">
                        {row.put_options.market_data.bid_price.toFixed(1)}/{row.put_options.market_data.ask_price.toFixed(1)}
                      </td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{formatNumber(row.put_options.market_data.oi)}</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{formatNumber(row.put_options.market_data.volume)}</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{row.put_options.option_greeks.delta.toFixed(2)}</td>
                      <td className="px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 md:py-2 text-gray-300 text-center text-[9px] sm:text-xs">{row.put_options.option_greeks.iv.toFixed(1)}%</td>
                    </tr>
                    ...
                    {shouldShowMarketPriceLine && (
                      <tr key={`market-price-line-${row.strike_price}`} className="border-none">
                        <td colSpan={13} className="px-0 py-0">
                          <div className="relative w-full h-8 sm:h-10 md:h-12 flex items-center">
                            {/* Horizontal line spanning the entire width */}
                            <div className="absolute w-full h-[1px] sm:h-[2px] bg-blue-500"></div>
                            
                            {/* Market price badge */}
                            <div className="absolute left-1/2 transform -translate-x-1/2 px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 bg-blue-600 rounded-full text-white text-[10px] sm:text-xs md:text-sm font-medium z-10 shadow-lg shadow-blue-500/20 border border-blue-400/30 whitespace-nowrap">
                              Market: {actualSpotPrice.toFixed(1)}
                            </div>
                          </div>
                        </td>
                      </tr>
