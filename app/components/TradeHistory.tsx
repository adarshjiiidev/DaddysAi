  ...
  const fetchLedger = async () => {
    ...
    try {
      ...
      if (response.data && response.data.ledger) {
        ...
        // Handle case where ledger data is missing but no error was thrown
        console.warn('Ledger data is missing or empty:', response.data);
        setLedger({});
        setExpandedDates({});
      }
    } catch (err) {
      ...
      let errorMessage = 'Failed to load trade history. Please try again.';
      
      if (axios.isAxiosError(err) && err.response?.data?.error) {
        const details = err.response.data.details || '';
        errorMessage = `${err.response.data.error}${details ? `: ${details}` : ''}`;
